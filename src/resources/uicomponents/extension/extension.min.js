...
c).replace("__total__",e));d(this).next().prop("checked",0<c)},a=function(){d(this).closest(".node",h).next("ul").find('input[type="checkbox"]').prop("checked",d(this).prop("checked"))},b=d(this).find("ul").prev(".node").addClass("parent");d(this).hasClass("collapsible")&&b.click(l);d(this).hasClass("selectable")&&(b.append('<span class="actions"><input type="checkbox"/></span>'),b.find('.actions input[type="checkbox"]').click(a).before('<span class="selectedCount"/>').prev(".selectedCount").each(f),
d(this).find('input[type="checkbox"]').click(function(){d(h).find(".selectedCount").each(f)}))};d(".document-tree").each(k);document.observe("xwiki:dom:updated",function(h){d(h.memo.elements).find(".document-tree").each(k)})});
require(["jquery"],function(d){var k=function(f){0<d(this).children(".ui-progress").size()?setTimeout(d.proxy(h,this),f||1E3):d(this).prev("form").find("button").prop("disabled",!1)},h=function(f){var a=d(this).prev("form").find("input[name=asyncURL]").prop("value");d.post(a,f||{},d.proxy(l,this)).fail(d.proxy(k,this,1E4))},l=function(f){f=d(this).hide().after(f).next(".extensionUpdater");d(this).remove();f.each(k).each(function(){document.fire("xwiki:dom:updated",{elements:[this]})});f.children(".extension-body-progress").find(".log-item-loading").each(function(){this.parentNode.scrollTop=
this.parentNode.scrollHeight})};d(".extensionUpdater").each(k).prev("form").find("button").click(function(f){f.preventDefault();0<d(this).parent(".dropdown-menu").size()&&(f=d(this).closest(".button-group").children(".dropdown-toggle"),f.prev().insertAfter(this),f.before(this));f=d(this).closest("form");f.find("button").prop("disabled",!0);var a={};a[this.name]=this.value;f.next(".extensionUpdater").each(d.proxy(h,null,a))})});
