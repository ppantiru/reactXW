...
function ts_resortTable(a){for(var b,c,d=0;d<a.childNodes.length;++d)a.childNodes[d].tagName&&"span"==a.childNodes[d].tagName.toLowerCase()&&(b=a.childNodes[d]);ts_getInnerText(b);var e=a.parentNode;d=e.cellIndex;e=getParent(e,"TABLE");if(!(1>=e.rows.length)){var g=ts_getInnerText(e.rows[getHeaderRow(e)+1].cells[d]);for(c=getHeaderRow(e)+2;!g;)g=ts_getInnerText(e.rows[c].cells[d]),++c;g||(g="");c=ts_sort_caseinsensitive;g.match(/^\d\d[\/-]\d\d[\/-]\d\d\d\d$/)&&(c=ts_sort_date);g.match(/^\d\d[\/-]\d\d[\/-]\d\d$/)&&
(c=ts_sort_date);g.match(/^[£$]/)&&(c=ts_sort_currency);g.match(/^[\d\.]+$/)&&(c=ts_sort_numeric);SORT_COLUMN_INDEX=d;d=[];g=[];for(var f=null!=getSortBottomRow(e)?getSortBottomRow(e)+1:e.rows.length;f<e.rows.length;++f)g[g.length]=e.rows[f];for(f=getHeaderRow(e)+1;f<e.rows.length;++f)d[d.length]=e.rows[f];d.sort(c);"down"==b.getAttribute("sortdir")?(c='<img border="0" src="'+image_path+image_up+'" alt="&#x2191;"/>',d.reverse(),b.setAttribute("sortdir","up")):(c='<img border="0" src="'+image_path+
image_down+'" alt="&#x2193;"/>',b.setAttribute("sortdir","down"));for(f=0;f<d.length;++f)(!d[f].className||d[f].className&&-1==d[f].className.indexOf("sortBottom"))&&e.tBodies[0].appendChild(d[f]);for(f=0;f<d.length;f++)d[f].className&&-1!=d[f].className.indexOf("sortBottom")&&e.tBodies[0].appendChild(d[f]);for(f=0;f<g.length;++f)e.tBodies[0].appendChild(g[f]);g=document.getElementsByTagName("span");for(d=0;d<g.length;++d)"sortarrow"==g[d].className&&getParent(g[d],"table")==getParent(a,"table")&&
(g[d].innerHTML='<img border="0" src="'+image_path+image_none+'" alt="&#x21F5;"/>');b.innerHTML=c;alternate(e)}}
