...
#end
#[[*/
'use strict';(function(l,c,f){require.config({paths:l.js});window.XWiki=window.XWiki||{};var g=XWiki.widgets=XWiki.widgets||{};if(XWiki.widgets.ModalPopup){var m=function(){return new g.JumpToPage};g.JumpToPage=Class.create(g.ModalPopup,{urlTemplate:(new XWiki.Document("__document__","__space__")).getURL("__action__"),initialize:function($super){var b=new Element("div");this.input=new Element("input",{type:"text",id:"jmp_target",title:c.inputTooltip});this.input.placeholder=this.input.title;b.appendChild(this.input);
this.viewButton=this.createButton("button",c.viewLabel,c.viewTooltip,"jmp_view");this.editButton=this.createButton("button",c.editLabel,c.editTooltip,"jmp_edit","secondary");var a=new Element("div",{"class":"buttons"});a.appendChild(this.viewButton);a.appendChild(this.editButton);b.appendChild(a);$super(b,{show:{method:this.showDialog,keys:f.show},view:{method:this.openDocument,keys:f.view,options:{propagate:!0}},edit:{method:this.openDocument,keys:f.edit}},{title:c.content,extraClassName:"jump-dialog",
verticalPosition:"top"});this.shortcuts.close.options={propagate:!0}},createDialog:function($super,b){Event.observe(this.viewButton,"click",this.openDocument.bindAsEventListener(this,"view"));Event.observe(this.editButton,"click",this.openDocument.bindAsEventListener(this,"edit"));$super(b);this.input.up(".xdialog-modal-container").addClassName("jump-dialog-container");var a=this;require(["jquery","xwiki-suggestPages"],function(d){l.css.selectize.forEach(function(e){d("<link/>").attr({type:"text/css",
rel:"stylesheet",href:e}).appendTo("head")});var p=function(e){var n=d(a.viewButton).add(a.editButton).find("input");!1===e?n.prop("disabled",!0):setTimeout(function(){n.prop("disabled",!1)},0)},h=function(e){p(""!==d(a.input).val())};d(a.input).on("change",h).suggestPages({maxItems:1});a.input.selectize.on("dropdown_open",d.proxy(p,null,!1));a.input.selectize.on("dropdown_close",h);a.input.selectize.focus();h()})},showDialog:function($super){$super();this.input.selectize?(this.input.selectize.clear(),
this.input.selectize.focus()):(this.input.value="",this.input.focus())},openDocument:function(k,b){this[(b||"view")+"Button"].down("input").disabled||(Event.stop(k),k=XWiki.Model.resolve(this.input.value,XWiki.EntityType.DOCUMENT,XWiki.currentDocument.documentReference),window.location=(new XWiki.Document(k)).getURL(b))}});XWiki.domIsLoaded&&m()||document.observe("xwiki:dom:loaded",m)}else console&&console.warn&&console.warn("[JumpToPage widget] Required class missing: XWiki.widgets.ModalPopup")}).apply("]]#",
