'use strict';window.MSCheckbox=Class.create({initialize:function(a,f,e,c,b,d){this.table=b;this.idx=d;this.table&&this.idx&&this.table.fetchedRows[this.idx]?(this.currentUorG=this.table.fetchedRows[this.idx].fullname,this.isUserInGroup=this.table.fetchedRows[this.idx].isuseringroup):(this.currentUorG=window.unregUser,this.isUserInGroup=!1);this.domNode=$(a);this.right=f;this.saveUrl=e;this.state=this.defaultState=c;this.states=[0,1,2];this.nrstates=this.states.length;this.images=["$xwiki.getSkinFile('js/xwiki/usersandgroups/img/none.png')",
"$xwiki.getSkinFile('js/xwiki/usersandgroups/img/allow.png')","$xwiki.getSkinFile('js/xwiki/usersandgroups/img/deny1.png')"];this.labels=["","",""];this.draw(this.state);this.attachEvents()},draw:function(a){for(;0<this.domNode.childNodes.length;)this.domNode.removeChild(this.domNode.firstChild);var f=document.createElement("img");f.src=this.images[a];this.domNode.appendChild(f);""!=this.labels[a]&&(f=document.createElement("span"),f.appendChild(document.createTextNode(this.labels[a])),this.domNode.appendChild(f))},
next:function(){this.state=(this.state+1)%this.nrstates;void 0!=this.table&&delete this.table.fetchedRows[this.idx];this.draw(this.state)},createClickHandler:function(a){return function(){if(!a.req){var f="",e=(a.state+1)%a.nrstates;if(a.currentUorG==window.currentUser)if(2==e){var c="$escapetool.javascript($services.localization.render('rightsmanager.denyrightforcurrentuser'))".replace("__right__",a.right);if(!confirm(c))if(c="$escapetool.javascript($services.localization.render('rightsmanager.clearrightforcurrentuserinstead'))".replace("__right__",
a.right),confirm(c))f="clear",a.state=2,e=0;else return}else{if(0==e&&(c="$escapetool.javascript($services.localization.render('rightsmanager.clearrightforcurrentuser'))".replace("__right__",a.right),!confirm(c)))return}else if(a.isUserInGroup||"XWiki.XWikiGuest"==window.currentUser&&"XWiki.XWikiAllGroup"==a.currentUorG)if(2==e){if(c="$escapetool.javascript($services.localization.render('rightsmanager.denyrightforgroup'))".replace(/__right__/g,a.right),c=c.replace("__name__",a.currentUorG),!confirm(c))if(c=
"$escapetool.javascript($services.localization.render('rightsmanager.clearrightforgroupinstead'))".replace(/__right__/g,a.right),c=c.replace("__name__",a.currentUorG),confirm(c))f="clear",a.state=2,e=0;else return}else{if(0==e&&(c="$escapetool.javascript($services.localization.render('rightsmanager.clearrightforgroup'))".replace(/__right__/g,a.right),c=c.replace("__name__",a.currentUorG),!confirm(c)))return}else if("admin"==a.right)if(2==e){if(c="$escapetool.javascript($services.localization.render('rightsmanager.denyrightforuorg'))".replace("__right__",
a.right),c=c.replace("__name__",a.currentUorG),!confirm(c))return}else if(0==e&&(c="$escapetool.javascript($services.localization.render('rightsmanager.clearrightforuorg'))".replace("__right__",a.right),c=c.replace("__name__",a.currentUorG),!confirm(c)))return;""==f&&(f=0==e?"clear":1==e?"allow":"deny");a.req=new Ajax.Request(a.saveUrl+"&action="+f+"&right="+a.right,{method:"get",onSuccess:function(b){"SUCCESS"==b.responseText.strip()?a.next():(alert("$services.localization.render('platform.core.rightsManagement.saveFailure')"),
b=unescape(window.location.pathname),window.location.href=b)},onFailure:function(){alert("$services.localization.render('platform.core.rightsManagement.ajaxFailure')")},onComplete:function(){delete a.req}})}}},attachEvents:function(){Event.observe(this.domNode,"click",this.createClickHandler(this))}});
function displayUsers(a,f,e,c){var b=a.userurl,d=a.usersaveurl,h=a.userinlineurl,g=a.wikiname,p=a.docurl,l=document.createElement("tr"),k=document.createElement("td");k.setAttribute("data-title","$escapetool.javascript($services.localization.render('xe.admin.users.username'))");if("local"==g){var m=document.createElement("a");m.href=b;m.appendChild(document.createTextNode(a.username));k.appendChild(m)}else k.appendChild(document.createTextNode(a.username));k.className="username";l.appendChild(k);
...
g.title="$services.localization.render('delete')",Event.observe(g,"click",deleteUserOrGroup(f,e,a.fullname,"group",c)),g.className="icon-manage",a=document.createElement("img"),a.src='$xwiki.getSkinFile("js/xwiki/usersandgroups/img/edit.png")',a.title="$services.localization.render('edit')",Event.observe(a,"click",editUserOrGroup(d,h,p)),a.className="icon-manage",b.appendChild(a),b.appendChild(g));l.appendChild(b);return l}
...
function setBooleanPropertyFromLiveCheckbox(a,f,e,c){return function(){var b="$xwiki.getURL('XWiki.XWikiPreferences', 'save')",d="XWiki.XWikiPreferences",h="0";void 0!=f&&0<f.length&&(b=f);void 0!=e&&0<e.length&&(d=e);void 0!=c&&(h=c);var g="yes",p="$xwiki.getSkinFile('js/xwiki/usersandgroups/img/allow-black.png')",l="1";"yes"==a.getAttribute("alt")&&(g="no",p="$xwiki.getSkinFile('js/xwiki/usersandgroups/img/none.png')",l="0");var k={parameters:{}};k.parameters[d+"_"+h+"_"+a.id]=l;k.parameters.ajax=
"1";k.parameters.comment="$services.localization.render('authenticate_viewedit_savecomment')";k.onSuccess=function(){a.alt=g;a.src=p};new Ajax.Request(b,k)}}
